...
type Tab = 'inicio' | 'dashboard' | 'sellers' | 'hunting' | 'rendimiento';
...
const RendPill = (props: { label: string; active: boolean; onClick: () => void }) => (
  <button onClick={props.onClick} style={{
    ...
    border: '1px solid ' + (props.active ? C.primary : C.border),
    background: props.active ? C.primaryLight : C.bgCard,
    color: props.active ? C.primaryDark : C.textSec,
    cursor: 'pointer', fontFamily: 'inherit',
  }}>{props.label}</button>
);

const AlertRow = (props: { sev: string; seller: string; type: string; d: string }) => (
  <div style={{ padding: '6px 0', borderBottom: '1px solid ' + C.borderLight, display: 'flex', gap: 8, alignItems: 'center', fontSize: 12 }}>
    <span>{props.sev === 'high' ? '\u{1F534}' : props.sev === 'med' ? '\u{1F7E1}' : '\u{1F7E2}'}</span>
    <div><div style={{ fontWeight: 600 }}>{props.seller}</div><div style={{ color: C.textMuted }}>{props.type}: {props.d}</div></div>
  </div>
);

const RTable = memo(function RTable(props: { columns: any[]; data: any[]; onRowClick?: (r: any) => void }) {
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead><tr style={{ background: C.bgAlt }}>{props.columns.map((c: any, i: number) => (
          <th key={i} style={{ textAlign: c.align || 'left', padding: '8px 8px', color: C.textMuted, borderBottom: '2px solid ' + C.border, fontWeight: 700, whiteSpace: 'nowrap', fontSize: 10, textTransform: 'uppercase' }}>{c.label}</th>
        ))}</tr></thead>
        <tbody>{props.data.map((r: any, ri: number) => (
          <tr key={ri} onClick={() => props.onRowClick && props.onRowClick(r)} className="row-hover" style={{ borderBottom: '1px solid ' + C.borderLight, cursor: props.onRowClick ? 'pointer' : undefined }}>
            {props.columns.map((c: any, ci: number) => (
              <td key={ci} style={{ padding: '7px 8px', color: C.text, textAlign: c.align || 'left', whiteSpace: 'nowrap' }}>{c.render ? c.render(r[c.key], r) : r[c.key]}</td>
            ))}
          </tr>
        ))}</tbody>
      </table>
...
const [perfLoading, setPerfLoading] = useState(false);
  ...
  const refreshPerf = useCallback(async () => {
    ...
    const oneYearAgo = (now.getFullYear() - 1) + '-01-01';
    try {
      const [agg, weekly] = await Promise.all([
        fetchPerformanceAgg(desde, hasta),
        fetchPerformanceWeekly(oneYearAgo, hasta),
      ]);
      if (agg.data) setPerfData(agg.data);
      if (weekly.data) setPerfWeekly(weekly.data);
    } catch (e) {
      console.log('Performance data not available yet:', e);
    }
        ...
        {tab === 'inicio' && (
          <div className="fi" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <KpiCard label="Sellers Activos" value={kpi.act} color={C.primary} sub={<span style={{ fontSize: 11, color: C.textMuted }}>{kpi.planCounts.Full} Full - {kpi.planCounts.Premium} Prem</span>} />
              <KpiCard label="Pausa" value={kpi.pausa} color={C.warning} />
              <KpiCard label="Fuga" value={kpi.fug} color={C.danger} />
              <KpiCard label="Pipeline" value={kpi.pipe} color={C.purple} />
              <KpiCard label="Revenue Mes" value={fmt(kpi.currentMonthRev)} color={C.primary} />
              <KpiCard label={'Rev. Proy. ' + CURRENT_YEAR} value={fmt(kpi.projectedRev)} color={C.primaryDark} />
            ...
            {perfData.length > 0 && (
              <div className="grid-3" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
                <div className="card" style={{ padding: 16 }}>
                  <h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>Metricas Rendimiento</h3>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12 }}>
                    {[
                      ['Fplus', perfTotals.avgFplus, Number(perfTotals.avgFplus) >= 4.8 ? C.primary : C.warning],
                      ['Content', perfTotals.avgContent, Number(perfTotals.avgContent) >= 85 ? C.primary : C.danger],
                      ['SP%', perfTotals.spRatio + '%', Number(perfTotals.spRatio) > 2 ? C.tertiary : C.danger],
                      ['OLT24', perfTotals.olt24tot + '%', Number(perfTotals.olt24tot) >= 30 ? C.cyan : C.danger],
                      ['Promo%', perfTotals.promoR + '%', Number(perfTotals.promoR) > 5 ? C.purple : C.danger],
                      ['Gen%', perfTotals.genS + '%', Number(perfTotals.genS) <= 5 ? C.primary : C.danger],
                    ].map(([l, v, c], i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px solid ' + C.borderLight }}>
                        <span style={{ color: C.textSec }}>{l as string}</span><strong style={{ color: c as string }}>{v as string}</strong>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="card" style={{ padding: 16 }}>
                  <h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>NMV Sellers</h3>
                  <div style={{ fontSize: 28, fontWeight: 800, color: C.primary }}>{fmt(perfTotals.nmv)}</div>
                  <div style={{ fontSize: 12, color: Number(perfTotals.yoy) > 0 ? C.primary : C.danger, fontWeight: 700 }}>YoY {Number(perfTotals.yoy) > 0 ? '+' : ''}{perfTotals.yoy}%</div>
                </div>
                <div className="card" style={{ padding: 16 }}>
                  <h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>Alertas</h3>
                  <div style={{ maxHeight: 140, overflowY: 'auto' }}>{perfAlerts.slice(0, 6).map((a: any, i: number) => <AlertRow key={i} sev={a.sev} seller={a.seller} type={a.type} d={a.d} />)}</div>
                </div>
              </div>
            )}

            {perfData.length === 0 && (
              <div className="card" style={{ padding: 24, textAlign: 'center', color: C.textMuted }}>
                <div style={{ fontSize: 32, marginBottom: 8 }}>📊</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>Sin datos de rendimiento</div>
                <div style={{ fontSize: 12 }}>Carga el CSV de BigQuery en seller_performance</div>
              </div>
            )}

            <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div className="card" style={{ padding: 16 }}>
                <h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>Funnel</h3>
              ...
              <div className="card" style={{ padding: 16 }}>
                <h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>Revenue {CURRENT_YEAR}</h3>
                <ResponsiveContainer width="100%" height={180}>
 ...
 {tab === 'rendimiento' && (
          <div className="fi" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', gap: 6 }}>
                <RendPill label="Vista General" active={rendSub === 'general'} onClick={() => { setRendSub('general'); setRendSellerPick(null); }} />
                <RendPill label="Por KAM" active={rendSub === 'kam'} onClick={() => { setRendSub('kam'); setRendSellerPick(null); }} />
                <RendPill label="Por Seller" active={rendSub === 'seller'} onClick={() => setRendSub('seller')} />
              </div>
              <div style={{ flex: 1 }} />
              {rendSub !== 'seller' && (
                <select value={rendKamFilter} onChange={(e) => setRendKamFilter(e.target.value)}>
                  <option value="Todas">Todas las carteras</option>
                  {Object.entries(KAM_POR_CATEGORIA).map(([cat, kam]) => <option key={cat} value={kam}>{cat} - {kam}</option>)}
                </select>
              )}
            </div>

            {perfLoading && <div style={{ textAlign: 'center', padding: 20, color: C.textMuted }}>Cargando...</div>}
            {!perfLoading && perfData.length === 0 && (
              <div className="card" style={{ padding: 32, textAlign: 'center', color: C.textMuted }}>
                <div style={{ fontSize: 40, marginBottom: 10 }}>📊</div>
                <div style={{ fontSize: 15, fontWeight: 700, color: C.text }}>Sin datos de rendimiento</div>
                <div style={{ fontSize: 12 }}>Carga el CSV de BigQuery en seller_performance</div>
              </div>
            )}

            {/* GENERAL */}
            {!perfLoading && perfData.length > 0 && rendSub === 'general' && (<>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <KpiCard label="NMV Total" value={fmt(perfTotals.nmv)} color={C.primary} sub={<span style={{ fontSize: 11, color: Number(perfTotals.yoy) > 0 ? C.primary : C.danger, fontWeight: 700 }}>YoY {Number(perfTotals.yoy) > 0 ? '+' : ''}{perfTotals.yoy}%</span>} />
                <KpiCard label="Sellers" value={perfTotals.n} color={C.tertiary} />
                <KpiCard label="Fplus" value={perfTotals.avgFplus} color={Number(perfTotals.avgFplus) >= 4.8 ? C.primary : C.warning} />
                <KpiCard label="Content" value={perfTotals.avgContent} color={Number(perfTotals.avgContent) >= 85 ? C.primary : C.danger} />
                <KpiCard label="SP%" value={perfTotals.spRatio + '%'} color={C.tertiary} />
                <KpiCard label="OLT24" value={perfTotals.olt24tot + '%'} color={C.cyan} />
                <KpiCard label="Promo%" value={perfTotals.promoR + '%'} color={C.purple} />
                <KpiCard label="Gen%" value={perfTotals.genS + '%'} color={Number(perfTotals.genS) <= 5 ? C.primary : C.danger} />
              </div>
              {weeklyChart.length > 0 && (<>
                <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <div className="card" style={{ padding: 16 }}><h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>Fplus Semanal</h3><ResponsiveContainer width="100%" height={160}><LineChart data={weeklyChart}><CartesianGrid strokeDasharray="3 3" stroke={C.border} /><XAxis dataKey="week" tick={{ fill: C.textMuted, fontSize: 9 }} /><YAxis domain={[4.5, 5]} tick={{ fill: C.textMuted, fontSize: 9 }} /><Tooltip contentStyle={ttS} /><Line dataKey="fplus" stroke={C.warning} strokeWidth={2} dot={{ r: 3, fill: C.warning }} /></LineChart></ResponsiveContainer></div>
                  <div className="card" style={{ padding: 16 }}><h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>SP Inv y Ratio</h3><ResponsiveContainer width="100%" height={160}><ComposedChart data={weeklyChart}><CartesianGrid strokeDasharray="3 3" stroke={C.border} /><XAxis dataKey="week" tick={{ fill: C.textMuted, fontSize: 9 }} /><YAxis yAxisId="l" tick={{ fill: C.textMuted, fontSize: 9 }} unit="M" /><YAxis yAxisId="r" orientation="right" tick={{ fill: C.textMuted, fontSize: 9 }} unit="%" /><Tooltip contentStyle={ttS} /><Bar yAxisId="l" dataKey="spInv" name="Inv SP" fill={C.tertiary} radius={[3, 3, 0, 0]} /><Line yAxisId="r" dataKey="spRatio" name="Ratio%" stroke={C.orange} strokeWidth={2} dot={{ r: 3 }} /></ComposedChart></ResponsiveContainer></div>
                </div>
                <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <div className="card" style={{ padding: 16 }}><h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>OLT 24h</h3><ResponsiveContainer width="100%" height={160}><BarChart data={weeklyChart}><CartesianGrid strokeDasharray="3 3" stroke={C.border} /><XAxis dataKey="week" tick={{ fill: C.textMuted, fontSize: 9 }} /><YAxis tick={{ fill: C.textMuted, fontSize: 9 }} unit="%" /><Tooltip contentStyle={ttS} /><Legend wrapperStyle={{ fontSize: 11 }} /><Bar dataKey="olt24fbf" name="FBF" stackId="a" fill={C.cyan} /><Bar dataKey="olt24fbs" name="FBS" stackId="a" fill={C.purple} radius={[3, 3, 0, 0]} /></BarChart></ResponsiveContainer></div>
                  <div className="card" style={{ padding: 16 }}><h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>Content Score</h3><ResponsiveContainer width="100%" height={160}><LineChart data={weeklyChart}><CartesianGrid strokeDasharray="3 3" stroke={C.border} /><XAxis dataKey="week" tick={{ fill: C.textMuted, fontSize: 9 }} /><YAxis domain={[80, 95]} tick={{ fill: C.textMuted, fontSize: 9 }} /><Tooltip contentStyle={ttS} /><Line dataKey="content" stroke={C.orange} strokeWidth={2} dot={{ r: 3, fill: C.orange }} /></LineChart></ResponsiveContainer></div>
                </div>
                <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <div className="card" style={{ padding: 16 }}><h3 style={{ margin: '0 0 10px', fontSize: 13, color: C.textSec, fontWeight: 700, textTransform: 'uppercase' }}>Promo Autogestionada</h3><ResponsiveContainer width="100%" height={160}><ComposedChart data={weeklyChart}><CartesianGrid strokeDasharray="3 3" stroke={C.border} /><XAxis dataKey="week" tick={{ fill: C.textMuted, fontSize: 9 }} /><YAxis yAxisId="l" tick={{ fill: C.textMuted, fontSize: 9 }} unit="M" /><YAxis yAxisId="r" orientation="right" tick={{ fill: C.textMuted, fontSize: 9 }} unit="%" /><Tooltip contentStyle={ttS} /><Legend wrapperStyle={{ fontSize: 11 }} /><Bar yAxisId="l" dataKey="promoOU" name="OU" stackId="a" fill={C.purple} /><Bar yAxisId="l" dataKey="promoFS" name="FS" stackId="a" fill={C.tertiary} radius={[3, 3, 0, 0]} /><Line yAxisId="r" dataKey="promoR" name="Ratio%" stroke={C.primary} strokeWidth={2} dot={{ r: 3 }} /></ComposedChart></ResponsiveContainer></div>
            ...
            {!perfLoading && perfData.length > 0 && rendSub === 'kam' && (<>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 14 }}>
                {perfKamStats.map((k: any) => (
                  <div key={k.kam} style={{ background: C.bgCard, border: '1px solid ' + C.borderLight, borderRadius: 14, padding: 16, borderLeft: '4px solid ' + C.primary, boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      <div><div style={{ fontSize: 14, fontWeight: 700 }}>{k.kam}</div><div style={{ fontSize: 11, color: C.textMuted }}>{k.sec} - {k.n} sellers</div></div>
                      <div style={{ textAlign: 'right' }}><div style={{ fontSize: 18, fontWeight: 800, color: C.primary }}>{fmt(k.nmv)}</div><div style={{ fontSize: 12, color: Number(k.grow) > 0 ? C.primary : C.danger, fontWeight: 700 }}>YoY {Number(k.grow) > 0 ? '+' : ''}{k.grow}%</div></div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 4, fontSize: 10 }}>
                      {[['Fplus', k.af, Number(k.af) >= 4.8 ? C.primary : C.warning], ['Content', k.ac, Number(k.ac) >= 85 ? C.primary : C.warning], ['SP%', k.spR + '%', C.tertiary], ['OLT24', k.olt24 + '%', C.cyan], ['Promo%', k.promoR + '%', C.purple], ['Gen%', k.genS + '%', Number(k.genS) <= 5 ? C.primary : C.danger]].map(([l, v, c]) => (
                        <div key={l as string} style={{ textAlign: 'center' }}><div style={{ color: C.textMuted }}>{l as string}</div><div style={{ fontWeight: 700, color: c as string }}>{v as string}</div></div>
                      ))}
                    </div>
                  </div>
                ))}
            ...
            {!perfLoading && perfData.length > 0 && rendSub === 'seller' && (<>
              <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                <select value={rendSellerPick ? rendSellerPick.sellerId : ''} onChange={(e) => { const s = enrichedPerf.find((x: any) => x.sellerId === e.target.value); setRendSellerPick(s || null); }} style={{ minWidth: 280 }}>
                  ...
                  {enrichedPerf.sort((a: any, b: any) => (a.seller_name || '').localeCompare(b.seller_name || '')).map((p: any) => <option key={p.sellerId} value={p.sellerId}>{p.seller_name} ({p.sec || p.kam}) - {p.sellerId}</option>)}
                </select>
